...
def upgrade() -> None:
    ...
    import_dataset.create(op.get_bind(), checkfirst=True)
    import_job_status.create(op.get_bind(), checkfirst=True)

    op.create_table(
        "import_jobs",
        sa.Column("id", sa.Integer(), nullable=False),
        sa.Column("dataset", import_dataset, nullable=False),
...
def downgrade() -> None:
    ...
    sa.Enum("preview_ready", "committed", "failed", name="import_job_status").drop(op.get_bind(), checkfirst=True)
    sa.Enum("room_master", "request_code_rules", "reservations", "inventory_overrides", name="import_dataset").drop(op.get_bind(), checkfirst=True)
